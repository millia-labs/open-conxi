...
This repo is fourteen Claude skills for hotels, plus 20 SOPs in `hotel-sops/sops/`. Deliverables are markdown, one HTML template, and small scripts. No build step for the skills.
...
- Every number that is a benchmark or a rule of thumb names its source in that skill's `references/evidence.md`, or, for an SOP, in its own Sources section. Nothing unsourced ships.

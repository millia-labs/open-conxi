...
3. In Claude Code: write `dashboard.html` and the merged `hotel-data.json` to the working folder, run `npx open-conxi tidy`, then open `dashboard.html`. Tell the GM that `npx open-conxi dashboard`, run in this folder, keeps the page current after every skill without re-running this one. In claude.ai: render the HTML as an artifact and print the merged `hotel-data.json` for the GM to save.

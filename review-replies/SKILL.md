...
4. For a miss, add a private note: the internal follow-up, the owner, the date. Link a review to a work order only when the paste gives the same room; never close or change a work order from a review, that is work-orders' job. Sign off on its own line as "Name, Title". When the GM asks the reply to claim something the paste does not show ("say we refunded him and fixed it"), write the honest draft first, then ask what is true.

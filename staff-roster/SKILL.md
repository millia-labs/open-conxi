...
Team message: end with a block headed "Team message" in plain text for the all_staff chat: under 600 characters, no tables, only what that team acts on today. In Claude Code, if hotel-profile.md names a all_staff chat under team_chats, save the block to team-message.txt and run `npx open-conxi team send --to all_staff --file team-message.txt`, which puts it in that Beeper chat as a draft for a person to check and send; add `--send` only when the GM asks for it to go straight out. Report what the command printed, and never say the message was drafted or sent unless it printed so. Otherwise, and in claude.ai, print the block for the GM to paste into the staff chat.
